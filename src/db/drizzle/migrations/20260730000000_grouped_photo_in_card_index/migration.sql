-- ---------------------------------------------------------------------------
-- Make the card_index has_photo flag group-aware.
--
-- Apple and other vCard 3.0 clients scope a property to an `itemN` group by
-- prefixing the name (`item1.PHOTO`), so an exact `name = 'PHOTO'` match misses
-- those cards: has_photo came out false and the contacts list rendered an
-- initials placeholder for a contact that does have a picture. Matching the
-- base name (the segment after the last `.`) mirrors `baseName()` in
-- src/data/vcard/prop.ts, which the rest of the codebase uses.
--
-- Only the PHOTO lookup changes; the remaining single-property lookups in this
-- function are left exactly as they were.
-- ---------------------------------------------------------------------------
...
BEGIN
    ...
    -- Presence-only photo flag: true iff a live PHOTO property exists, whether
    -- plain (`PHOTO`) or group-scoped (`item1.PHOTO`). We never read the value
    -- (it may be a multi-hundred-KB base64 data: URI).
    SELECT EXISTS(
        SELECT 1 FROM dav_property
        WHERE component_id = v_comp_id
          AND deleted_at IS NULL
          AND (name = 'PHOTO' OR name LIKE '%.PHOTO')
    ) INTO v_has_photo;
...
-- Replay the trigger over existing vCards so grouped photos are picked up. On a
-- fresh DB this is a no-op.
DO $$
DECLARE
    r RECORD;
BEGIN
