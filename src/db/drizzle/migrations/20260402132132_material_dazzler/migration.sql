...
ALTER TABLE "group" DROP CONSTRAINT "group_primary_name_group_name_id_fkey";--> statement-breakpoint
DROP TABLE "casbin_rule";--> statement-breakpoint
DROP TABLE "group_name";--> statement-breakpoint
ALTER TABLE "auth_user" DROP CONSTRAINT "auth_user_auth_source_auth_id_unique";--> statement-breakpoint
...
ALTER TABLE "dav_collection" DROP CONSTRAINT "dav_collection_collection_type_check", ADD CONSTRAINT "dav_collection_collection_type_check" CHECK ((collection_type = ANY (ARRAY['collection'::text, 'calendar'::text, 'addressbook'::text, 'inbox'::text, 'outbox'::text])));