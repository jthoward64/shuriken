-- ---------------------------------------------------------------------------
-- Extend the card_index trigger to also extract BDAY into data->>'bday'.
--
-- The Birthday-calendar feature (see services/birthday/) needs a cheap query
-- for "every contact in collection X with a non-empty BDAY". We index BDAY
-- as a normalized YYYY-MM-DD-or-MM-DD string in the existing JSONB data
-- column so the existing GIN index covers it; no new column required.
--
-- vCard 4 BDAY accepts:
--   * full date         19850412   / 1985-04-12
--   * yearless date     --0412
--   * date-time         19850412T120000Z (rare for birthdays)
-- We normalize:
--   YYYYMMDD       -> YYYY-MM-DD
...
-- Birthday consumers tolerate both year-bearing and yearless forms.
-- ---------------------------------------------------------------------------
...
-- Backfill BDAY for existing card_index rows by replaying the trigger on
