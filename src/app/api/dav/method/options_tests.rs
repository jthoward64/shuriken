...
async fn options_sets_dav_header() {
    ...
    assert!(
        dav_value.contains("calendar-access"),
        "Should support CalDAV"
    );
...
async fn options_collection_includes_mkcol_methods() {
    ...
    let allow_value = allow_header.unwrap().to_str().unwrap();
    assert!(
        allow_value.contains("MKCALENDAR"),
        "Collections should allow MKCALENDAR"
    );
    assert!(
        allow_value.contains("MKCOL"),
        "Collections should allow MKCOL"
    );
}
...
async fn options_item_excludes_collection_methods() {
    ...
    assert!(
        !allow_value.contains("MKCALENDAR"),
        "Items should not allow MKCALENDAR"
    );
    assert!(
        !allow_value.contains("MKCOL"),
        "Items should not allow MKCOL"
    );
    assert!(
        allow_value.contains("OPTIONS"),
        "Items should allow basic methods"
    );
    assert!(allow_value.contains("GET"));
    assert!(allow_value.contains("DELETE"));
}
