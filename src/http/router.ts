...
import { embedHandler, isEmbedPath } from "#src/http/embed/handler.tsx";
import { feedHandler, isFeedPath } from "#src/http/feed/handler.ts";
import { resolveForwardedUrl } from "#src/http/forwarded-url.ts";
import { applySecurityHeaders } from "#src/http/security-headers.ts";
import {
	HTTP_BAD_REQUEST,
...
import type { TrashService } from "#src/services/trash/service.ts";
...
export const handleRequest = (
	...
	return Effect.gen(function* () {
		...
		// Fetched once, up front, so both the pre-auth public routes below and
		// the rest of dispatch share a single config snapshot.
		...
		// Public unauthenticated calendar embed widget — same rationale as the
		// feed endpoint above, plus a feature-level gate (each calendar also
		// opts in individually via `embed_enabled`, checked inside embedHandler).
		if (isEmbedPath(url.pathname)) {
			if (!cfg.embed.calendarWidgetEnabled) {
				return new Response("Not Found", { status: HTTP_NOT_FOUND });
			}
			return yield* embedHandler(req, url);
		}
		...
		// Applied to every response regardless of which branch produced it
		// (DAV, UI, feed, embed, or the error mapper above) — see
		// security-headers.ts. `cfg` is a cheap Context lookup, not a fresh
		// env read (Config is resolved once at layer-build time).
		Effect.flatMap((response) =>
			Effect.map(AppConfigService, (cfg) => {
				const scheme = resolveForwardedUrl(
					url,
					req.headers,
					clientIp,
					cfg.auth.trustedProxies,
