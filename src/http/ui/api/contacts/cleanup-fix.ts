...
const errorFragment = (message: string) =>
	renderFragment("partials/cleanup-suggestion-error", { message });
...
export const contactsCleanupFixHandler = (
	...
	AclService | ContactCleanupService | InstanceService | TemplateService
> =>
	Effect.gen(function* () {
		...
		return yield* cleanup.applyFix(instanceId, fixResult.fix).pipe(
			Effect.flatMap(() =>
				renderFragment("partials/cleanup-suggestion-done", {
					contactFn: form.get("contactFn")?.toString() ?? "",
				}),
			),
