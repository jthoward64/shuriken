...
// The selected ids the caller may read; selections that no longer exist are dropped
const authorizeSelection = Effect.fn("ui.contacts.bulkDownload.authorize")(
	function* (principalId: PrincipalId, ids: ReadonlyArray<InstanceId>) {
		...
		for (const id of ids) {
			...
			const collectionId = Option.map(instanceOpt, (inst) =>
				CollectionId(inst.collectionId),
			);
			if (Option.isNone(collectionId)) {
				continue;
			}
			yield* acl.check(
				principalId,
...
> =>
	Effect.gen(function* () {
		...
		const authorized = yield* authorizeSelection(principal.principalId, ids);
