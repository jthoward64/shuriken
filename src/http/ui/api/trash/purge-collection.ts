...
// ---------------------------------------------------------------------------
// POST /ui/api/trash/collections/:collectionId/purge
// ---------------------------------------------------------------------------

export const trashPurgeCollectionHandler = (
	_req: Request,
	...
	Effect.gen(function* () {
		...
		yield* trashSvc
			.purgeCollectionForever(collectionId, principal.principalId)
			.pipe(
				Effect.catchTags({
					TrashNotFound: () =>
					...
					TrashNotOwner: () =>
						Effect.fail(
							forbidden(undefined, "Not the owner of this collection"),
						),
				}),
			);
