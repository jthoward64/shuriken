...
import { HTTP_NOT_FOUND, HTTP_SEE_OTHER } from "#src/http/status.ts";
...
const stubLayers = Layer.mergeAll(
	...
	Layer.succeed(UserService, {
		...
		findById: () =>
			Effect.succeed({
				...
				user: {},
			} as unknown as UserWithPrincipal),
		findBySlug: die,
		update: die,
		delete: die,
		addCredential: die,
	...
	Layer.succeed(CollectionRepoTag, {
		...
		findBySlug: die,
		listByOwner: die,
		listDeletedByOwner: die,
		listDeletedOlderThan: die,
	...
	Layer.succeed(BirthdayService, {
		regenerate: die,
	}),
	Layer.succeed(TaskEditService, {
		create: die,
		update: die,
		delete: die,
	}),
	...
	Layer.succeed(TrashService, {
		listTrash: () => Effect.succeed({ collections: [], instances: [] }),
		restoreCollection: die,
		restoreInstance: die,
	...
	Layer.succeed(BulkJobRepository, {
		...
		fail: die,
		clearBlob: die,
		listExpiredBlobs: die,
		failStaleRunning: die,
	}),
...
describe("uiRouter", () => {
	it("redirects / to the calendar (the default page)", async () => {
		const res = await run("/");
		expect(res.status).toBe(HTTP_SEE_OTHER);
		expect(res.headers.get("Location")).toBe("/ui/calendar");
	});

	it("redirects /ui to the calendar (the default page)", async () => {
