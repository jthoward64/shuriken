...
// ---------------------------------------------------------------------------
// GET /ui/contacts/export.vcf?addressbook=<collectionId>
//
// Streams the address book's contents as a concatenated VCF stream.
// Requires DAV:read on the collection.
// ---------------------------------------------------------------------------
...
> =>
	Effect.gen(function* () {
		...
		const body = yield* exportAddressBookToVcf(collectionId);
		const bytes = new TextEncoder().encode(body);
		const filename = `${sanitize(filenameHint)}.vcf`;
