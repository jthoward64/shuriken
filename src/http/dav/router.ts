...
import { davError, notFound } from "#/domain/errors.ts";
...
const parseDavPath = (
  url: URL,
): Effect.Effect<ResolvedDavPath, DavError, DavServices> => {
  ...
  return Effect.gen(function* () {
    ...
    if (!principalRow) {
      return yield* Effect.fail(notFound(`Principal not found: ${principalSlug}`));
    }
    ...
    switch (req.method.toUpperCase()) {
      ...
      default:
        return new Response(null, {
          status: 405,
          headers: { Allow: "OPTIONS, GET, HEAD, PUT, DELETE, PROPFIND, PROPPATCH, MKCOL, REPORT" },
        });
    }
  }).pipe(
