...
import { ClientJsServiceLive } from "#src/http/ui/client/index.ts";
import { CssServiceLive } from "#src/http/ui/css/index.ts";
...
import { TrashPurgeLayer } from "#src/services/trash/purge.live.ts";
...
const BaseAppLayer = Layer.mergeAll(
	...
	BulkJobRepositoryLive.pipe(Layer.provide(InfraLayer)),
...
export const AppLayer = Layer.mergeAll(
	...
	TrashServiceFull,
	// Trash purge sweep fiber — no-op when trash.retentionDays === 0 (deletes
	// are already immediate hard-deletes in that mode; see TrashPurgeLayer).
	...
	// Bulk-job sweep fiber — reconciles jobs abandoned by a process restart and
	// evicts expired result blobs (contacts export/bulk-download .vcf bytes).
	BulkJobSweepLayer.pipe(Layer.provide(BaseAppLayer)),
);
...
export { CssService } from "#src/http/ui/css/index.ts";
export { PageCacheService } from "#src/http/ui/page-cache/index.ts";
...
export { TaskEditService } from "#src/services/task-edit/service.ts";
export {
...
export { TrashService } from "#src/services/trash/service.ts";
export {
	UserRepository,
	UserService,
