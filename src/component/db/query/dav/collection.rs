...
mod tests {
    ...
    #[test]
    fn by_principal_filters_correctly() {
        let principal_id = uuid::Uuid::new_v4();
        let _query = by_principal(principal_id);
        // Query should build with principal filter
    }

    #[test]
    fn by_principal_not_deleted_applies_both_filters() {
        let principal_id = uuid::Uuid::new_v4();
        let _query = by_principal_not_deleted(principal_id);
        // Query should build with both principal and deleted_at filters
    }

    #[test]
    fn by_uri_and_principal_filters_both() {
        let principal_id = uuid::Uuid::new_v4();
        let _query = by_uri_and_principal("/calendar/home", principal_id);
        // Query should build with both uri and principal filters
    }
}
