	...
	listMembers: (groupId) =>
		Effect.succeed(
			[...(stores.memberships.get(groupId) ?? new Set())].flatMap((userId) => {
				...
				const autoAssignedBy =
					stores.membershipAutoAssignedBy.get(`${groupId}:${userId}`) ?? null;
				return [{ user: userRow, principal, autoAssignedBy }];
			}),
		),
	...
	listByMember: (userId) =>
		Effect.succeed(
			[...stores.groups.entries()].flatMap(([groupId, groupRow]) => {
				...
				const autoAssignedBy =
					stores.membershipAutoAssignedBy.get(`${groupId}:${userId}`) ?? null;
				return [{ principal, group: groupRow, autoAssignedBy }];
			}),
		),
	...
	const self: TestEnvBuilder = {
		...
		toLayer() {
			...
			const birthdayServiceLayer = Layer.succeed(BirthdayService, {
				regenerate: () =>
					Effect.succeed({ inserted: 0, updated: 0, deleted: 0 }),
			});
