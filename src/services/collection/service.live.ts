...
export const CollectionServiceLive = Layer.effect(
	...
	Effect.gen(function* () {
		...
		return {
			...
			hardDelete: Effect.fn("CollectionService.hardDelete")(function* (
				id: CollectionId,
			) {
				yield* Effect.annotateCurrentSpan({ "collection.id": id });
				yield* Effect.logTrace("collection.hardDelete", { id });
				yield* repo.findById(id).pipe(
					Effect.flatMap(someOrNotFound(`Collection not found: ${id}`)),
					Effect.flatMap(() => repo.hardDelete(id)),
				);
				yield* Effect.logDebug("collection.hardDelete: purged", { id });
			}),
