		...
		return ShareLinkService.of({
			...
			> =>
				Effect.gen(function* () {
					...
					yield* Effect.forEach(input.calendars, (c) =>
						repo.addCalendar(row.id, c.calendarId, c.visibility),
					);
			...
			setVisibility: (
				...
				Effect.gen(function* () {
					yield* loadOwned(id, caller);
					yield* repo.setCalendarVisibility(id, calendarId, visibility);
				}),
			...
			addCalendar: (
				...
				Effect.gen(function* () {
					...
					yield* repo.addCalendar(id, calendarId, visibility);
				}),
