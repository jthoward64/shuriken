...
export const InstanceServiceLive = Layer.effect(
	...
	Effect.gen(function* () {
		...
		return {
			...
			hardDelete: Effect.fn("InstanceService.hardDelete")(function* (
				id: InstanceId,
			) {
				yield* Effect.annotateCurrentSpan({ "instance.id": id });
				yield* Effect.logTrace("instance.hardDelete", { id });
				yield* repo.findById(id).pipe(
					Effect.flatMap(someOrNotFound(`Instance not found: ${id}`)),
					Effect.flatMap(() => repo.hardDelete(id)),
				);
				yield* Effect.logDebug("instance.hardDelete: purged", { id });
			}),
