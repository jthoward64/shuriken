...
// ---------------------------------------------------------------------------
// GroupService.syncOidcMembership
// ---------------------------------------------------------------------------

describe("GroupService.syncOidcMembership", () => {
	it("adds membership (auto-assigned) when a claimed group matches oidcGroups", async () => {
		const env = makeTestEnv();
		const groupId = crypto.randomUUID();
		const userId = crypto.randomUUID();
		env
			.withGroup({ id: groupId, oidcGroups: ["engineering"] })
			.withUser({ id: userId });
		...
		expect(env.stores.memberships.get(groupId)?.has(userId)).toBe(true);
		expect(
			env.stores.membershipAutoAssignedBy.get(`${groupId}:${userId}`),
		).toBe("oidc");
	});

	it("removes a previously auto-assigned membership when the claim no longer matches", async () => {
		const env = makeTestEnv();
		const groupId = crypto.randomUUID();
		const userId = crypto.randomUUID();
		env
			...
			.withUser({ id: userId });
		env.stores.memberships.set(groupId, new Set([userId]));
		env.stores.membershipAutoAssignedBy.set(`${groupId}:${userId}`, "oidc");
	...
	it("leaves a manually-assigned membership untouched even when the claim doesn't match", async () => {
		const env = makeTestEnv();
		const groupId = crypto.randomUUID();
		const userId = crypto.randomUUID();
		env
