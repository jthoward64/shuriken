...
describe("GroupRepository.update (integration)", () => {
	...
	it("create with oidcGroups then update persist and round-trip on group.oidcGroups", async () => {
		const result = await runSuccess(
			GroupRepository.pipe(
				Effect.flatMap((r) =>
					Effect.gen(function* () {
						const created = yield* r.create({
							slug: Slug("sre"),
							oidcGroups: ["sre", "on-call"],
						});
						const updated = yield* r.update(GroupId(created.group.id), {
							oidcGroups: ["sre-only"],
						});
						const found = yield* r.findById(GroupId(created.group.id));
		...
		expect(result.updated.group.oidcGroups).toEqual(["sre-only"]);
		const found = Option.getOrThrow(result.found);
		expect(found.group.oidcGroups).toEqual(["sre-only"]);
	});
});
...
describe("GroupRepository membership (integration)", () => {
	...
	it("addMember with an autoAssignedBy source persists and round-trips via listMembers/listAutoAssignedGroupIds", async () => {
		const result = await runSuccess(
			Effect.gen(function* () {
				const userRepo = yield* UserRepository;
				const groupRepo = yield* GroupRepository;
				const { user } = yield* userRepo.create({
					slug: Slug("erin"),
					displayName: "Erin",
					email: Email("erin@example.com"),
					credentials: [],
				});
				const { group } = yield* groupRepo.create({ slug: Slug("sre-2") });
				const groupId = GroupId(group.id);
				const userId = UserId(user.id);
				yield* groupRepo.addMember(groupId, userId, "oidc");
				const members = yield* groupRepo.listMembers(groupId);
				const autoAssignedGroupIds = yield* groupRepo.listAutoAssignedGroupIds(
					userId,
					"oidc",
				);
		...
		expect(result.autoAssignedGroupIds).toHaveLength(1);
	});
});
