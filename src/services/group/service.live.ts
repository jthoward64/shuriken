	...
	Effect.gen(function* () {
		...
		return {
			...
			addMember: Effect.fn("GroupService.addMember")(function* (
				groupId: GroupId,
				userId: UserId,
				autoAssignedBy: AutoAssignedBySource | null = null,
			) {
				...
				const result = yield* repo.findById(groupId).pipe(
					...
					Effect.flatMap(() => repo.addMember(groupId, userId, autoAssignedBy)),
				);
			...
			syncOidcMembership: Effect.fn("GroupService.syncOidcMembership")(
				function* (userId: UserId, claimedGroups: ReadonlyArray<string>) {
					yield* Effect.annotateCurrentSpan({ "user.id": userId });
					yield* Effect.logTrace("group.syncOidcMembership", {
						userId,
						claimedGroups,
					});

					const claimed = new Set(claimedGroups);
					const [allGroups, autoAssignedGroupIds] = yield* Effect.all([
						repo.list(),
						repo.listAutoAssignedGroupIds(userId, OIDC_AUTO_ASSIGNED_BY),
					]);

					const shouldBeMember = allGroups.filter((g) =>
						g.group.oidcGroups.some((name) => claimed.has(name)),
					);
					const shouldBeMemberIds = new Set(
						shouldBeMember.map((g) => g.group.id),
					);
					...
					yield* Effect.forEach(
						...
						(g) =>
							repo.addMember(
								g.group.id as GroupId,
								userId,
								OIDC_AUTO_ASSIGNED_BY,
							),
						{ discard: true },
					);

					const toRemove = autoAssignedGroupIds.filter(
						(groupId) => !shouldBeMemberIds.has(groupId),
					);
					yield* Effect.forEach(
						toRemove,
						(groupId) => repo.removeMember(groupId, userId),
						{ discard: true },
					);

					yield* Effect.logDebug("group.syncOidcMembership done", {
						userId,
						added: shouldBeMember.length,
