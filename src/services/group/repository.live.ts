		...
		const groupRows = yield* runDbQuery((db) =>
			db
				...
				.values({
					...
					oidcGroups: input.oidcGroups ? [...input.oidcGroups] : undefined,
				})
				.returning(),
		);
...
const update = Effect.fn("GroupRepository.update")(
	function* (
		...
		if (input.oidcGroups !== undefined) {
			const oidcGroups = [...input.oidcGroups];
			yield* runDbQuery((db) =>
				db
					.update(group)
					.set({ oidcGroups, updatedAt: sql`now()` })
...
const addMember = Effect.fn("GroupRepository.addMember")(
	function* (
		...
		return yield* runDbQuery((db) =>
			db
				.insert(membership)
				.values({ groupId, userId, autoAssignedBy })
				.onConflictDoNothing(),
		).pipe(Effect.asVoid);
	},
	Effect.tapError((e) =>
				...
				rows.map(
					(r) =>
						({
							principal: r.principal,
							user: r.user,
							autoAssignedBy: r.membership.autoAssignedBy,
...
const listAutoAssignedGroupIds = Effect.fn(
	"GroupRepository.listAutoAssignedGroupIds",
)(
	function* (userId: UserId, source: AutoAssignedBySource) {
		yield* Effect.annotateCurrentSpan({ "user.id": userId, source });
		yield* Effect.logTrace("repo.group.listAutoAssignedGroupIds", {
			userId,
			source,
		});
		return yield* runDbQuery((db) =>
			db
				...
				.where(
					and(
						...
						eq(membership.autoAssignedBy, source),
					),
				),
		).pipe(Effect.map((rows) => rows.map((r) => r.groupId as GroupId)));
	},
	Effect.tapError((e) =>
		Effect.logWarning("repo.group.listAutoAssignedGroupIds failed", e.cause),
	),
);
...
export const GroupRepositoryLive = Layer.effect(
	...
	Effect.gen(function* () {
		...
		return {
			...
			listAutoAssignedGroupIds: (
				...args: Parameters<typeof listAutoAssignedGroupIds>
			) => run(listAutoAssignedGroupIds(...args)),
