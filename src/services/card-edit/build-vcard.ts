...
/** TYPE + LABEL parameters for a typed value. */
const typedParams = (
	types: ReadonlyArray<string>,
	label: string | undefined,
): ReadonlyArray<IrParameter> => [...typeParams(types), ...labelParams(label)];

export const emailProp = (tv: ContactTypedValue): IrProperty => ({
	name: "EMAIL",
	parameters: [...typedParams(tv.types, tv.label)],
...
/** `Family;Given;Additional;Prefix;Suffix` from the form's two components. */
export const nValue = (family: string, given: string): string =>
	`${family};${given};;;`;
...
export const adrProp = (addr: ContactAddress): IrProperty => ({
	name: "ADR",
	parameters: [...typedParams(addr.types, addr.label)],
...
export const buildVcardComponent = (
	...
	if (form.familyName !== "" || form.givenName !== "") {
		props.push(textProp("N", nValue(form.familyName, form.givenName)));
	}
	if (form.nickname !== "") {
