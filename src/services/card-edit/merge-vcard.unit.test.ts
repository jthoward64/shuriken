...
describe("mergeFormIntoVcard", () => {
	...
	it("edits a grouped Apple email in place, keeping group + X-ABLabel + params", () => {
		...
		const out = mergeFormIntoVcard(
			...
			form({
				fn: "A",
				emails: [{ value: "new@x.com", types: ["home"], preferred: false }],
			}),
			UID,
		);
	...
	it("appends a newly added email as a bare property", () => {
		...
		const out = mergeFormIntoVcard(
			...
			form({
				fn: "A",
				emails: [{ value: "a@x.com", types: ["work"], preferred: false }],
			}),
			UID,
		);
	...
	it("preserves N additional/prefix/suffix via parse→edit→merge", () => {
		const existing = vcard([
			prop("VERSION", "4.0"),
			prop("UID", UID),
			prop("FN", "A"),
	...
	it("overwrites N straight from the form (no hidden preservation)", () => {
		const existing = vcard([
			prop("VERSION", "4.0"),
			prop("UID", UID),
			prop("FN", "A"),
		...
		expect(getText(find(out, "N"))).toBe("Jones;Jane;;;");
	});
	...
	it("keeps multiple grouped emails paired positionally", () => {
		...
		const out = mergeFormIntoVcard(
			...
			form({
				fn: "A",
				emails: [
					{ value: "a@x.com", types: [], preferred: false },
					{ value: "B2@x.com", types: [], preferred: false },
				],
			}),
			UID,
		);
	...
	it("edits grouped TEL and URL in place, keeping groups", () => {
		...
		const out = mergeFormIntoVcard(
			...
			form({
				fn: "A",
				tels: [{ value: "+1 999", types: ["work"], preferred: false }],
