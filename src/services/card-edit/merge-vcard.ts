...
const withTypedValue = (
	...
	...prop,
	parameters: [
		...prop.parameters.filter((p) => p.name !== "TYPE" && p.name !== "LABEL"),
		...typeParams(types),
		...labelParams(label),
	],
...
// N: replace only Family (0) and Given (1); keep Additional/Prefix/Suffix.
const mergeNValue = (
	existing: string,
	family: string,
	given: string,
	...
	while (parts.length < N_COMPONENTS) {
		parts.push("");
	}
	parts[0] = family;
	parts[1] = given;
...
const updateMulti = (
	...
	switch (base) {
		...
		case "TEL": {
			...
			return withTypedValue(prop, tv.value, tv.types, tv.label);
		}
		case "URL":
		...
		case "ADR": {
			...
			return withTypedValue(prop, addressJoined(addr), addr.types, addr.label);
		}
		case "SOCIALPROFILE":
		case "IMPP":
...
const hasSingle = (base: Single, form: ContactFormData): boolean => {
	switch (base) {
		case "FN":
			return form.fn !== "";
		case "N":
			return form.familyName !== "" || form.givenName !== "";
		case "KIND":
			return form.kind !== "";
		case "NICKNAME":
...
const singleValue = (base: Single, form: ContactFormData): IrValue => {
	switch (base) {
		...
		case "N":
			return text(nValue(form.familyName, form.givenName));
		case "BDAY":
...
const updateSingle = (
	...
	if (base === "N") {
		return withValue(prop, {
			type: "TEXT",
			value: mergeNValue(getText(prop), form.familyName, form.givenName),
		});
	}
	return withValue(prop, singleValue(base, form));
};
