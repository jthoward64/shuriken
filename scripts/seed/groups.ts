 ...
 * get real coverage.
 */
export const seedGroup = Effect.fn("seed.group")(function* (
	index: number,
	spec: {
	...
	const groupId = GroupId(created.group.id);
	const principalId = PrincipalId(created.principal.id);
	...
	const eventCount = Math.floor(
		MIN_SHARED_CALENDAR_EVENTS +
			Math.random() * (MAX_SHARED_CALENDAR_EVENTS - MIN_SHARED_CALENDAR_EVENTS),
	);
	yield* seedEvents(
		sharedCalendar.id as CollectionId,
		eventCount,
...
const grantGroupAce = Effect.fn("seed.grantGroupAce")(function* (
