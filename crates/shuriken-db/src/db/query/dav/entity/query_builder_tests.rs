...
fn test_entity_by_logical_uid_query_builds() {
    ...
    assert!(
        ...
        "by_logical_uid() query should be valid"
    );
}
...
fn test_all_components_query_builds() {
    ...
    assert!(
        query_is_valid(query),
        "components_for_entity() query should be valid"
    );
}
...
fn test_properties_for_component_query_builds() {
    ...
    assert!(
        query_is_valid(query),
        "properties_for_component() query should be valid"
    );
}
...
fn test_parameters_for_property_query_builds() {
    ...
    assert!(
        query_is_valid(query),
        "parameters_for_property() query should be valid"
    );
}
...
fn test_components_for_entity_filter() {
    ...
    let query_str =
        diesel::debug_query::<diesel::pg::Pg, _>(&components_for_entity(entity_id)).to_string();
...
fn test_properties_for_component_filter() {
    let component_id = uuid::Uuid::new_v4();
    let query_str =
        diesel::debug_query::<diesel::pg::Pg, _>(&properties_for_component(component_id))
            .to_string();
...
fn test_parameters_for_property_filter() {
    let property_id = uuid::Uuid::new_v4();
    let query_str =
        diesel::debug_query::<diesel::pg::Pg, _>(&parameters_for_property(property_id)).to_string();
...
fn test_different_entity_ids_produce_different_queries() {
    ...
    assert_ne!(
        query1, query2,
        "Different entity IDs should produce different query parameters"
    );
}
