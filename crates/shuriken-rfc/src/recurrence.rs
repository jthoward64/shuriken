...
pub fn extract_recurrence_data_with_resolver(
    ...
    let mut rrule_set = rrule.build(dt_start).map_err(|err| match err {
        rrule::RRuleError::ValidationError(validation) => {
            RfcError::RRuleValidationError(validation)
        }
        _ => RfcError::ValidationError(err.to_string()),
    })?;
...
pub fn ical_datetime_to_utc_with_resolver(
    ...
    match &dt.form {
        crate::rfc::ical::core::DateTimeForm::Utc => {
            Some(DateTime::<Utc>::from_naive_utc_and_offset(naive, Utc))
        }
        crate::rfc::ical::core::DateTimeForm::Floating => {
            if let Some(tzid_str) = tzid {
                // Convert using timezone
                crate::rfc::ical::expand::convert_to_utc(naive, tzid_str, resolver).ok()
            } else {
                // Treat as floating time (interpret as UTC)
                Some(DateTime::<Utc>::from_naive_utc_and_offset(naive, Utc))
            }
        }
        crate::rfc::ical::core::DateTimeForm::Zoned { tzid: dt_tzid } => {
            // Use the TZID from the datetime form
            crate::rfc::ical::expand::convert_to_utc(naive, dt_tzid, resolver).ok()
        }
    }
}
